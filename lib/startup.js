...
export function environmentSummary(env = process.env) {
  ...
  return {
    ...
    leadStore: (env.LEAD_STORE ?? 'true').toLowerCase() !== 'false',
    // Whether the dashboard exists at all. Without a password every /admin
    // route answers 404, so this line is the quickest way to tell a
    // "dashboard introuvable" report from a wrong URL.
    admin: has('ADMIN_PASSWORD') || has('ADMIN_PASSWORD_HASH'),
    adminSessionSecret: has('ADMIN_SESSION_SECRET'),
...
export function describeStartupError(err) {
  ...
  if (err?.code === 'ERR_MODULE_NOT_FOUND' || err?.code === 'MODULE_NOT_FOUND') {
    lines.push(
      'cause probable : un fichier manque sur le serveur. Vérifiez que lib/, assets/',
      '                 et vendor/ ont bien été envoyés avec la même arborescence.',
    );
  }
