...
export function createStore({ dir = join(ROOT, 'data'), maxEvents = MAX_EVENTS, maxAudit = 5000 } = {}) {
  ...
  return {
    ...
    /**
     * Can the store actually write?
     *
     * The one thing /healthz could not previously answer. Every write path
     * swallows its error on purpose — a full disk must not cost a prospect who
     * has already filled in the form, and an analytics beacon must never
     * surface as an error on the page — so a data directory that is missing or
     * read-only produces an empty dashboard and nothing else. This turns that
     * silence into one word.
     *
     * Returns a status, never the path: /healthz is public.
     */
    async writable() {
      try {
        await ensureDir();
      ...
      try {
        // Being writable is not enough: DATA_DIR pointing at a regular file
        // passes access(W_OK) and then fails every append with ENOTDIR — a
        // green health check on a store that records nothing.
        if (!(await stat(dir)).isDirectory()) return 'pas un répertoire';
