...
export function createStore({ dir = join(ROOT, 'data'), maxEvents = 20000 } = {}) {
  ...
  return {
    ...
    async stats({ days = 30 } = {}) {
      ...
      const views = periodEvents.filter((e) => e.name === 'page_view');
      const pageViews = views.length;
      // Sessions, not page views, are the honest denominator: one prospect who
      // reloads three times is one opportunity to convert, not three.
      ...
      return {
        ...
        byEventDevice: tally(periodEvents.filter((e) => e.name === 'page_view'), (e) => e.device || '—'),
        pageViews,
        sessions,
        // Leads over sessions — the only rate the recorded data supports.
        // `null`, not 0, when nothing was recorded: no traffic data means no
        // rate to state, and a 0 % would read as a catastrophe rather than as
        // "the tracking endpoint has not been reached yet".
        conversionRate: sessions > 0 ? +((periodLeads.length / sessions) * 100).toFixed(2) : null,
        daily: (() => {
          const out = {};
