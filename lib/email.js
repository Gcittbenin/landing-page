...
import { emailHtml, emailText, describeProject } from './format.js';
 ...
 *
 * Never throws, for the same reason as the WhatsApp channel.
 *
 * @returns {Promise<{ok: boolean, skipped?: string, messageId?: string, error?: string, status?: number}>}
 */
export async function sendLeadEmail(lead, config, { fetchImpl = fetch } = {}) {
  const { apiKey, to, from, provider, subject } = config;
