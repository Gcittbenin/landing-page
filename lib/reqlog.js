 ...
 * this process.
 *
 * ── Per process, and that matters ──────────────────────────────────────────
 *
 * These live in this module's scope, so there is one set per Node process.
 * Passenger keeps a pool of them and picks one per request: a beacon can be
 * counted by one worker while /healthz is answered by another, which reports
 * zero. So a zero here is evidence, not proof — read it together with the
 * `pid` /healthz also returns. What every worker does share is the file on
 * disk, which is why data/events.jsonl and the console are the authority.
 *
