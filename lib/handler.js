...
import { countStoreError } from './reqlog.js';
...
/** Same reasoning as the limiter: one store per process, opened lazily. */
let sharedStore = null;
export function getStore(config) {
  if (!config.store.enabled) return null;
  if (!sharedStore) {
    sharedStore = createStore(config.store.dir ? { dir: config.store.dir } : {});
  }
  return sharedStore;
}

/** Test seam: drop the retained limiter and store state. */
...
export async function handleLead(req, { env = process.env, fetchImpl = fetch, now = Date.now() } = {}) {
  ...
  // ---- request context ----------------------------------------------------
  // Derived server-side, never taken from the body: the browser can lie about
  // its own user agent, but it cannot lie about which socket the request came
  // from, and a client-supplied "ip" field would be worthless evidence.
  const agent = parseUserAgent(header('user-agent'));
  const lead = {
    ...result.lead,
    // 'unknown' is the rate limiter's bucket key, not an address; blank it so
    // the email and the CRM simply omit the row instead of printing a word.
    ip: ip === 'unknown' ? '' : ip,
    userAgent: agent.userAgent,
    device: agent.device,
    browser: agent.browser,
    os: agent.os,
    // No `referer` is recorded: the form posts with fetch(), so the header is
    // always our own page. Where the prospect actually came from is captured
    // client-side as `source` / `sourceDetail` / the UTM fields.
  };

  // ---- persistence --------------------------------------------------------
  // Before the fan-out: if the process dies mid-send, the lead is still on
  // disk. A store failure is logged and swallowed — a full disk must not cost
  // us a prospect who has already filled in the form.
  let stored = null;
  const store = getStore(config);
  if (store) {
    try {
      stored = await store.addLead(lead);
    } catch (error) {
      countStoreError();
      // Also to logs/startup.log: Passenger discards stdout, and a prospect
      // lost to a full disk must not be a silent event.
      logStartup(`[lead] ÉCHEC ENREGISTREMENT : ${error?.code ?? ''} ${error?.message || error}`);
      console.error('[lead] échec enregistrement :', error?.message || error);
    }
  }
