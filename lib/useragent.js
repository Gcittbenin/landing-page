...
const BROWSERS = [
  ...
  [/\bFirefox\/([\d.]+)/, 'Firefox'],
  [/\bCriOS\/([\d.]+)/, 'Chrome'],
  // "HeadlessChrome/…" has no word boundary before "Chrome", so the generic
  // rule below never sees it and the UA used to fall through to Safari.
 ...
 *
 * Two of these deliberately skip the leading word boundary, because the
 * strings that actually appear in the wild are "HeadlessChrome" and
 * "Googlebot" / "bingbot" / "AhrefsBot" — none of which has a boundary before
 * the token. `bot\b` still requires one after, so it does not fire on a word
 * that merely starts with those letters.
 */
const BOT_RE =
  /headless|bot\b|\b(?:crawler|spider|crawl|slurp|facebookexternalhit|preview|monitor|curl|wget|python-requests|axios|okhttp)\b/i;
