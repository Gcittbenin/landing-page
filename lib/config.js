...
/**
 * Same as num(), but 0 is a real value rather than "unset".
 *
 * MIN_FILL_MS=0 has to mean "no minimum fill time" — it is the documented way
 * to switch the timing check off in a test environment. With num() it fell
 * through to the 3000 ms default and every fast submission was silently
 * dropped as spam, which is exactly the failure nobody notices.
 */
...
export function loadConfig(env = process.env) {
  return {
    ...
    security: {
      ...
      minFillMs: numOrZero(env.MIN_FILL_MS, 3000),
    },
    ...
    // Local prospect store, read by the /admin dashboard. On by default: it is
    // the only copy of a lead that survives an email or WhatsApp outage.
    // DATA_DIR must point at a writable path — on a read-only filesystem
    // (serverless), set it to a temp dir or switch the store off.
    store: {
      enabled: (env.LEAD_STORE ?? 'true').toLowerCase() !== 'false',
      dir: env.DATA_DIR ?? '',
    },
  };
}
