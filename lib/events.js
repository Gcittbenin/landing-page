...
// A real visit emits a dozen events. This ceiling stops a script from filling
// the disk while leaving an enthusiastic prospect well clear of it.
const EVENT_LIMIT = { max: 80, windowMs: 10 * 60 * 1000 };
...
export async function handleEvent(req, { store } = {}) {
  ...
  try {
    ...
    console.warn('[event] non enregistré :', error?.message || error);
  }
