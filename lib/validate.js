...
export function validateLead(body, { now = Date.now(), minFillMs = 0 } = {}) {
  ...
  return {
    ...
    lead: {
      ...
      // The browser's session id, the join between this prospect and the
      // pages they read before writing to us. Random and session-scoped, so
      // it identifies a visit, never a person across visits.
      sid: clean(input.sid, MAX.short),
